...
void App::finish() {
        ...
        while (resolved_count < m_plugins.size()) {
            ...
            for (; resolved_count < wave_end; ++resolved_count) {
                ...
                for (const auto& requirement :
                     m_plugins[resolved_count].requirements) {
                    if (m_plugin_indices.contains(requirement.key)) {
                        continue;
                    }
                    ...
                    if (existing == missing.end()) {
                        ...
                        requirement.configured && !existing->configured
                    ) {
            ...
            for (auto& requirement : missing) {
                ...
                m_plugin_indices.emplace(requirement.key, dependency_index);
                m_plugins.push_back(
                    PluginEntry {
                        .key = requirement.key,
                        .implementation_type = requirement.implementation_type,
    ...
    const auto rebuild_plugin_order = [&] {
        ...
        std::function<void(std::size_t)> visit = [&](std::size_t index) {
            ...
            for (const auto& requirement : m_plugins[index].requirements) {
                visit(m_plugin_indices.at(requirement.key));
            }
            stack.pop_back();
    ...
    try {
        std::size_t priority_start = 0;
        while (true) {
            m_plugin_registry_frozen = true;
            resolve_dependencies();
            rebuild_plugin_order();
            const auto registered_count = m_plugins.size();

            m_plugin_registry_frozen = false;
            bool added_plugins = false;
            const auto setup_plugins = [&](bool priority_only) {
                for (auto index : m_plugin_order) {
                    if (priority_only && index < priority_start) {
                        continue;
                    }
                    if (m_plugins[index].state != PluginState::Registered) {
                        continue;
                    }
                    m_plugins[index].state = PluginState::SettingUp;
                    auto* plugin = m_plugins[index].plugin.get();
                    plugin->setup(*this);
                    m_plugins[index].state = PluginState::Setup;
                    if (m_plugins.size() != registered_count) {
                        return true;
                    }
                }
                return false;
            ...
            if (priority_start != 0) {
                added_plugins = setup_plugins(true);
            }
            if (!added_plugins) {
                added_plugins = setup_plugins(false);
            }
            if (!added_plugins) {
                break;
            }
            priority_start = registered_count;
        }
