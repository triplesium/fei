...
TEST_CASE(
    ...
    auto dependency = compile_luau_script_module(
        LuauScriptSource {
            .name = "execution_pool_dependency.luau",
            .content = R"(
    ...
    LuauExecutionPool pool(2);
    auto loaded_dependency = pool.load_module(*dependency);
    REQUIRE(loaded_dependency);
    REQUIRE(pool.active_module_count() == 1);

    auto invalid_dependency =
        std::make_shared<LuauExecutionModule>(*loaded_dependency);
    invalid_dependency->lanes[1] = invalid_luau_script_module_id;
    const LuauExecutionImportBinding invalid_import {
        .specifier = "./helpers",
        .module = invalid_dependency,
    };
    ...
    const LuauExecutionImportBinding valid_import {
        .specifier = "./helpers",
        .module = std::make_shared<LuauExecutionModule>(*loaded_dependency),
    };
    ...
    REQUIRE(pool.unload_module(*loaded_dependency));
    REQUIRE(pool.active_module_count() == 0);
}
