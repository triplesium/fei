...
TEST_CASE(
    ...
    const std::string plugin_source = R"(
    ...
    auto script = luau_assets(world).emplace(plugin_source);

    auto artifact = compile_luau_script_module(
        LuauScriptSource {
            .name = "registry_plugins.luau",
            .content = plugin_source,
        }
    );
    REQUIRE(artifact);
    auto prepared =
        std::make_shared<LuauScriptModuleArtifact>(std::move(*artifact));
    REQUIRE(prepared->plugins.size() == 2);

    auto& scripts = luau_scripts(world);
    scripts.queue_plugin(script, prepared, "DebugPlugin");
    apply_luau_script_queue(world);

    REQUIRE(scripts.queue_errors().size() == 1);
    CHECK(scripts.queue_errors()[0].error.message.contains(
        "must be installed before"
    ));
    REQUIRE(scripts.size() == 0);

    scripts.queue_plugin(script, prepared, "CorePlugin");
    scripts.queue_plugin(script, prepared, "DebugPlugin");
    apply_luau_script_queue(world);
...
TEST_CASE(
    "LuauScriptSystemRegistry installs a prepared Plugin artifact",
    "[scripting_luau][system][registry][plugin][prepared]"
) {
    register_luau_registry_test_types();
    World world;
    add_luau_script_system_resources(world);
    ...
    auto artifact = compile_luau_script_module(
        LuauScriptSource {
            .name = "prepared_plugin.luau",
            .content = increment_source,
        }
    );
    REQUIRE(artifact);

    auto modified = luau_assets(world).modify(script);
    REQUIRE(modified);
    modified->set_content("this is not valid Luau");

    auto& scripts = luau_scripts(world);
    scripts.queue_plugin(
        script,
        std::make_shared<LuauScriptModuleArtifact>(std::move(*artifact)),
        "CounterPlugin"
    );
    ...
    REQUIRE(scripts.queue_errors().empty());
    REQUIRE(scripts.find_asset(script, "CounterPlugin"));
    world.run_schedule(Update);
    CHECK(world.resource<LuauRegistryCounter>().value == 4);
}

TEST_CASE(
    "LuauAssetCompiler caches one artifact per script asset",
    "[scripting_luau][compiler][asset][cache]"
) {
    register_luau_registry_test_types();
    World world;
    add_luau_script_system_resources(world);
    auto script = luau_assets(world).emplace(increment_source);
    LuauAssetCompiler compiler {
        luau_assets(world),
        world.resource<AssetServer>(),
    };

    auto first = compiler.compile_module(script);
    REQUIRE(first);
    auto second = compiler.compile_module(script);
    REQUIRE(second);
    CHECK(first->get() == second->get());
    REQUIRE((*first)->plugins.size() == 1);
    CHECK((*first)->plugins.front().name == "CounterPlugin");
}
