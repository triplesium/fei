...
class ContainerAdapter {
  public:
    ...
    virtual const IndexedContainerAdapter* indexed() const { return nullptr; }
    virtual IndexedContainerAdapter* indexed() { return nullptr; }
...
class IndexedContainerAdapter : public ContainerAdapter {
  public:
    const IndexedContainerAdapter* indexed() const override { return this; }
    IndexedContainerAdapter* indexed() override { return this; }
    ...
    // Runtime-schema containers derive their element type from the supplied
    // instance. Static C++ containers use the instance-independent default.
    virtual Result<TypeId, ContainerError> element_type(Ref container) const {
        (void)container;
        return element_type();
    }
    ...
    virtual Result<TypeId, ContainerError>
    element_type(Ref container, std::size_t index) const {
        (void)container;
    ...
    virtual Result<Ref, ContainerError>
    at(Ref container, std::size_t index) const {
        (void)container;
        (void)index;
        return failure(unsupported_error("at"));
    }
    ...
    virtual Status<ContainerError>
    insert(Ref container, std::size_t index, Ref value) const {
        (void)index;
        (void)value;
        return unsupported(container, "insert");
    }

    virtual Status<ContainerError>
    erase(Ref container, std::size_t index) const {
        (void)index;
        return unsupported(container, "erase");
    }
};
...
class AssociativeContainerAdapter : public ContainerAdapter {
  public:
    ...
    virtual Result<TypeId, ContainerError> key_type(Ref container) const {
        (void)container;
        return key_type();
    }
    virtual TypeId mapped_type() const { return {}; }
    virtual Result<TypeId, ContainerError> mapped_type(Ref container) const {
        (void)container;
        return mapped_type();
    }
    ...
    virtual Status<ContainerError> clear(Ref container) const = 0;

    virtual Status<ContainerError>
    insert(Ref container, AssociativeElementRef entry) const = 0;

    virtual Result<Ref, ContainerError> find(Ref container, Ref key) const {
        (void)container;
        (void)key;
        return failure(unsupported_error("find"));
    }

    virtual Result<bool, ContainerError>
    contains(Ref container, Ref key) const {
        (void)container;
        (void)key;
        return failure(unsupported_error("contains"));
    }

    virtual Status<ContainerError> erase(Ref container, Ref key) const {
        (void)key;
        return unsupported(container, "erase");
    }
};
