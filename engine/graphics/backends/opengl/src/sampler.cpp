...
inline Color4F to_color(SamplerBorderColor border_color) {
    switch (border_color) {
        ...
        default:
            ets::fatal("Unsupported SamplerBorderColor");
            return {};
    }
}
...
void SamplerOpenGL::create_gl_resource() const {
    ETS_GL_CALL(glGenSamplers(1, &m_sampler));

    ETS_GL_CALL(glSamplerParameteri(
        m_sampler,
        GL_TEXTURE_WRAP_S,
    ...
    if (m_desc.address_mode_u == SamplerAddressMode::ClampToBorder ||
        ...
        ETS_GL_CALL(glSamplerParameterfv(
            m_sampler,
...
void SamplerOpenGL::destroy_gl_resource() {
    if (m_sampler != 0) {
        ETS_GL_CALL(glDeleteSamplers(1, &m_sampler));
        m_sampler = 0;
    }
}
