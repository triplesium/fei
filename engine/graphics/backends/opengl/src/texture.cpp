...
void TextureOpenGL::create_gl_resource() const {
    ETS_PROFILE_SCOPE("OpenGL Texture Create");
    if (m_sample_count != TextureSampleCount::Count1) {
        ets::fatal("OpenGL texture MSAA storage is not implemented yet");
    }

    auto gl_target = to_gl_texture_target(m_texture_usage, m_texture_type);

    ETS_GL_CALL(glCreateTextures(gl_target, 1, &m_texture));

    if (gl_target == GL_TEXTURE_1D) {
        ETS_GL_CALL(glTextureStorage1D(
            m_texture,
        ...
        ETS_GL_CALL(glTextureStorage2D(
            m_texture,
            to_gl_sizei(m_mip_level),
            m_gl_sized_internal_format,
            to_gl_sizei(m_width),
        ...
        ETS_GL_CALL(glTextureStorage3D(
            m_texture,
            to_gl_sizei(m_mip_level),
            m_gl_sized_internal_format,
            to_gl_sizei(m_width),
        ...
        ets::fatal("Unsupported texture type for OpenGL");
        return;
    }
}
...
void TextureOpenGL::destroy_gl_resource() {
    if (m_texture != 0) {
        ETS_GL_CALL(glDeleteTextures(1, &m_texture));
        m_texture = 0;
    }
}
