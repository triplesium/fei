...
export function createImageGenerationTools(invoke: ImageGenerationInvoker): AgentTool<any>[] {
    return [{
        ...
        description: "Generate a new image from a detailed text prompt and optional references (existing project PNG paths), saving a PNG project asset. Use resolution and aspect_ratio for dimensions, or size for explicit pixels. Omitted options use configured defaults. seed requires OpenRouter. Choose a new path inside assets/; existing files are never overwritten. This calls a paid image API and may take several minutes. Returns the saved path and dimensions. Do not automatically retry a failed or cancelled request; it may already have incurred a charge.",
