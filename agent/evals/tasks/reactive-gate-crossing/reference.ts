...
export async function reference(invoke: Invoke) {
    const source = await readFile(join(taskDirectory("reactive-gate-crossing"), "reference/controller.luau"), "utf8");
    ...
    await invoke("native_play_segment", { interface: capabilityInterface, source, max_ticks: 180 });
