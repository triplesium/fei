...
std::unique_ptr<Mesh> MeshFactory::create_capsule(
    ...
    for (std::uint32_t i = 0; i <= rings; ++i) {
        float phi =
            (static_cast<float>(i) / static_cast<float>(rings)) * (PI * 0.5f);
...
std::unique_ptr<Mesh> MeshFactory::create_plane(
    ...
    float dx = width / static_cast<float>(subdivisions);
    float dz = depth / static_cast<float>(subdivisions);

    for (std::uint32_t z = 0; z <= subdivisions; ++z) {
        for (std::uint32_t x = 0; x <= subdivisions; ++x) {
            float px = -half_width + (static_cast<float>(x) * dx);
            float pz = -half_depth + (static_cast<float>(z) * dz);
...
std::unique_ptr<Mesh> MeshFactory::create_arrow(
    ...
    shaft_length = std::max(0.0f, shaft_length);
    ...
    for (std::uint32_t i = 0; i <= segments; ++i) {
        ...
        float hyp = std::sqrt(
            (head_length * head_length) + (head_radius * head_radius)
        );
